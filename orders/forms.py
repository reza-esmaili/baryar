...
class CargoRequestForm(forms.ModelForm):
    ...
    container_count = forms.IntegerField(
        ...
        required=False, 
        label="تعداد کانتینر",
        widget=forms.NumberInput(attrs={'class': 'form-control', 'placeholder': 'مثال: 2'})
    )
    ...
    actual_weight = forms.DecimalField(
        required=False,
        label="وزن واقعی (kg)",
        widget=forms.NumberInput(attrs={'class': 'form-control', 'placeholder': 'مثال: 150'})
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'origin_province' in self.data:
            try:
                ...
                self.fields['origin_city'].queryset = City.objects.filter(province_id=province_id)
        ...
        if 'destination_country' in self.data:
            try:
                ...
                self.fields['destination_city'].queryset = DestinationCity.objects.filter(country_id=country_id)
            except (ValueError, TypeError):
                pass
